...
p = re.compile([[
	...
	statement      <- if_else_statement / if_statement / while_statement / function_call_statement / assign_statement
	block          <- open_brace statement_list close_brace / statement

	assign_statement        <- ({:tag: '' -> 'assign_statement':}        {:identifier: identifier:} '=' ws? {:expression: expression:} semicolon) -> {}
	function_call_statement <- ({:tag: '' -> 'function_call_statement':} {:function_call: function_call:} semicolon) -> {}
	if_statement            <- ({:tag: '' -> 'if_statement':}            'if' ws? open_parens {:expression: expression:} close_parens {:block: block:}) -> {}
	if_else_statement       <- ({:tag: '' -> 'if_else_statement':}       'if' ws? open_parens {:expression: expression:} close_parens {:if_block: block:} 'else' ws? {:else_block: block:}) -> {}
	while_statement         <- ({:tag: '' -> 'while_statement':}         'while' ws? open_parens {:expression: expression:} close_parens {:block: block:}) -> {}
	...
	primary     <- integer / function_call / variable / open_parens expression close_parens
